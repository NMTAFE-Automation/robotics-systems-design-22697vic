...
`cv2.GaussianBlur()` smooths an image by convolving it with a Gaussian kernel, averaging each pixel with its neighbors weighted by distance. It is applied as a noise-reduction preprocessing step before edge detection or thresholding, since raw sensor noise would otherwise create many false edges and contours. It is especially useful ahead of Canny edge detection in classic vision pipelines. The result is a softened image where fine noise is suppressed while overall shapes and larger features remain visible, improving the reliability of later processing.
